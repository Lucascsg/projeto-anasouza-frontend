...
function configurarBotaoCompra(produto) {
    ...
    btnComprar.addEventListener('click', function(event) {
        ...
        // 1. Encontra qual seletor de cor está ativo AGORA
        const seletorAtivo = document.querySelector('.seletor-cor.active');
        let corSelecionada = seletorAtivo ? seletorAtivo.title : produto.variacoes[0].cor;

        // 2. Encontra a variação completa (com imagem, cor, etc.)
        const variacaoSelecionada = produto.variacoes.find(v => v.cor === corSelecionada);
        
        // 3. Monta o item para o carrinho com a cor CORRETA
