...
function carregarCarrinho() {
    ...
    carrinho.forEach(item => {
        // --- CÁLCULO DO PREÇO CORRIGIDO ---
        const precoUnitario = parsePrecoParaNumero(item.preco); // Usa a nova função
        
        if (!isNaN(precoUnitario)) { // Verifica se a conversão deu certo
...
function atualizarContadorCarrinho() {
    let carrinho = JSON.parse(localStorage.getItem('carrinho')) || [];
    const totalItens = carrinho.reduce((total, item) => total + item.quantidade,);
