...
function configurarEventosCarrinho() {
    ...
    if (btnFinalizar) {
        btnFinalizar.addEventListener('click', (event) => {
            ...
            // --- ATUALIZAÇÃO PARA SALVAR NO HISTÓRICO ---
            ...
            // --- FIM DA ATUALIZAÇÃO DE HISTÓRICO ---


            // Salva o pedido finalizado no sessionStorage para a página de agradecimento
            sessionStorage.setItem('ultimoPedido', JSON.stringify(carrinho));
            // Limpa o carrinho atual do localStorage
...
function formatarMoeda(valor) {
    // Adicionada verificação para garantir que o valor é numérico
...
/**
 * Calcula o valor total de um pedido (array de itens do carrinho).
 * @param {Array} itens - O array de itens do pedido.
 * @returns {number} - O valor total do pedido.
 */
function calcularTotalPedido(itens) {
    return itens.reduce((total, item) => {
        // Usa a mesma lógica segura de conversão de preço
        const precoNumero = parseFloat(String(item.preco).replace('R$ ', '').replace(',', '.'));
